function minWindow(s: string, t: string): string {
  ...
  while (right < s.length) {
    ...
    // 10. Once all required characters are present in the window (i.e., `have === need`), attempt to shrink the window.
    while (have === need) {
...
function minSubStrWindow(s: string, t: string): string {
  if (t.length > s.length) return "";
  ...
  return minLength === Infinity
    ? ""
    : s.substring(minStartIdx, minStartIdx + minLength);
}
