...
function heapify(type: "min" | "max", heap: number[], targetIdx: number) {
  let currNodeAsParent = targetIdx;

  const left = 2 * targetIdx + 1;
  const right = 2 * targetIdx + 2;

  if (type === "min") {
    // Compare the left child with the current parent
    if (left < heap.length && heap[left] < heap[currNodeAsParent]) {
      currNodeAsParent = left;
    }
    // Compare the right child with the current parent
    if (right < heap.length && heap[right] < heap[currNodeAsParent]) {
      currNodeAsParent = right;
    }
  }
  ...
  // If targetIdx was changed, swap and recursively heapify the affected subtree
  if (currNodeAsParent !== targetIdx) {
    [heap[currNodeAsParent], heap[targetIdx]] = [
      heap[targetIdx],
      heap[currNodeAsParent],
    ];
    heapify(type, heap, currNodeAsParent);
  }
}
